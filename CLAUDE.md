...
- **Run / test / lint (exact commands):** `npm run dev` (dev server), `npm run build` (production build → `dist/`), `npm run preview` (preview the build). **No test suite, no linter configured** — don't invent `npm test`/`npm run lint` commands that don't exist; if you add tests or a linter, wire the script into `package.json` and document it here.
...
- **The one gotcha that bites everyone:** The daily check-in only ever targets *today*. `saveEntry()` in `App.jsx` already dedupes by an arbitrary `entry.date`, so it's tempting to assume backdating a missed day works if you're only looking at the data layer — there is currently no UI path to it at all. This is the top item in `PRODUCT.md`'s Known Gaps (tagged P0).
