...
- **Non-obvious conventions:**
  ...
  - Brand colors live in `tailwind.config.js` (`primary`, `primary-dark`, `primary-light`, `accent`, `bg` — renamed from `vert`/`vert-dark`/`vert-light`/`jaune`/`creme` during the Option B + Style 3 design migration, see `design.md`) — use these for chrome/brand elements. Never reuse them for clinical status. Severity/status color is a *second*, separate system — `success`/`warning`/`danger` Tailwind scales plus the 5-step gradient unified in `src/utils/severityColors.js` (single source of truth, consumed by `LikertScale.jsx` and `Report.jsx`'s `dimColor`/`wearOffColor`/`heatClass`) — extend that module for any new severity-color need instead of hardcoding a new mapping or a raw Tailwind default (`red-500`, `green-600`, etc.).
  - Fonts: Atkinson Hyperlegible (`font-sans`, the default — body, labels, forms) + Lora (`font-display` — page titles, report headline, narrative heading only) + JetBrains Mono (`font-mono` — times, counts, dosage, unchanged). See `design.md` → Typography for the named type scale.
  - Structural icons are SVG components in `src/components/icons.jsx` (1.75px stroke, `currentColor`), not emoji — see `design.md` → Icons. Plain Unicode punctuation used as a structural mark (`✓`/`+` in `DayStrip`, `☐` in the report's checklist) is not in scope for that rule.
