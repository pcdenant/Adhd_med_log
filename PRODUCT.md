...
- ~~**[P2]** ~17 instances of sub-AA gray-on-white text; inconsistent status-color logic~~ — **shipped**: muted text moved to `gray-500`+, inactive Likert colors darkened, and the previously-duplicated status-color logic (`dimColor`/`wearOffColor`/`heatClass` in `Report.jsx` vs. `LikertScale.jsx`'s own hardcoded gradient) is now unified in `src/utils/severityColors.js`, the single source of truth both consume.
- ~~**[P2]** Touch targets under 44px (severity pills, header edit-mode controls, especially the icon-only pencil toggle with no padding at all)~~ — **shipped**: the pencil button gets an invisible padding+negative-margin hit-area expansion to 44×44 with zero visual size change; header edit-mode inputs/buttons and the side-effect severity pills sized well above the WCAG 2.5.8 AA 24px floor, as close to the 44px mobile-HIG ideal as the compact header/nested-accordion layout allows.
- ~~**[P2]** Repeated uppercase-tracked "eyebrow" heading pattern (an explicit AI-tell) across the accordion headers, DayStrip, and the report's narrative caption~~ — **shipped**: each of the three locations got a distinct, context-appropriate treatment instead of one uniform kicker — see `ARCHITECTURE.md` → Component notes.
- ~~**[P3]** Zero responsive breakpoints; the day-of-week heatmap got tight under ~360px~~ — **shipped**: `DayOfWeekSection`'s 7-column heatmap uses mobile-first base sizing with a `sm:` breakpoint restoring the original sizing at ≥640px — the app's second use of `sm:` after `DayStrip`'s summary text.
